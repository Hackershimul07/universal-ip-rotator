#!/bin/bash
# ==============================================================================
#                 👑 SHIMUL UNIVERSAL IP ROTATOR INSTALLER 👑
...
echo -e "${MAGENTA}                 Developed by: SHIMUL                 ${NC}"
echo -e "${BLUE}══════════════════════════════════════════════════════${NC}\n"
...
echo -e "${CYAN}Protected by Shimul Cloudflare Edge Security System${NC}\n"
...
if [ "$AUTH_SUCCESS" -ne 1 ]; then
    ...
    echo -e "${YELLOW}  Please contact Admin SHIMUL for an active license key. ${NC}"
    echo -e "${RED}${BOLD}══════════════════════════════════════════════════════${NC}\n"
    exit 1
fi
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
...
INSTALL_DIR="/opt/shimul-ip-rotator"
...
curl -sSL "https://raw.githubusercontent.com/hackershimul07/universal-ip-rotator/main/bot.py" -o "${INSTALL_DIR}/bot.py"
...
cat << EOF > /etc/systemd/system/shimul-ip-rotator.service
[Unit]
Description=Shimul Universal IP Route and Rotator Bot
...
systemctl enable shimul-ip-rotator.service
systemctl restart shimul-ip-rotator.service --no-block

sleep 3

if systemctl is-active --quiet shimul-ip-rotator.service; then
    echo -e "\n${GREEN}${BOLD}======================================================${NC}"
    echo -e "${GREEN}${BOLD}   🎉 UNIVERSAL INSTALLATION COMPLETED BY SHIMUL!    ${NC}"
    echo -e "${GREEN}${BOLD}======================================================${NC}"
    echo -e "${CYAN}• Supported Protocols:${NC} ${YELLOW}OpenVPN, WireGuard, VLESS, VMess, Trojan, Shadowsocks, SOCKS5, HTTP${NC}"
    echo -e "${CYAN}• Bot Service:${NC} ${GREEN}Active & Running (systemctl status shimul-ip-rotator)${NC}"
    ...
    echo -e "\n${RED}⚠️ Service started with warnings. Check logs: journalctl -u shimul-ip-rotator -e${NC}\n"
fi
